...
{
    // Check for nested-hold report flag
    if (cm1.send_nested_hold_report) {
        cm1.send_nested_hold_report = false;  // Clear the flag

        // Temporarily switch cm pointer to cm1 so report reads correct state
        cmMachine_t *saved_cm = cm;
        cm = &cm1;

        // **CHANGED: Use STAT 6 for both transitions, vary hold state**
        cmMachineState saved_machine_state = cm1.machine_state;
        cmFeedholdState saved_hold_state = cm1.hold_state;

        // **FIRST REPORT: STAT 6 with hold:0 (transition from running)**
        cm1.machine_state = MACHINE_CYCLE;           // ← STAT 5 or 6 depending on motion
        cm1.hold_state = FEEDHOLD_OFF;               // ← hold:0

        _populate_unfiltered_status_report();
        nv_print_list(STAT_OK, TEXT_MULTILINE_FORMATTED, JSON_OBJECT_FORMAT);

        // **SECOND REPORT:  STAT 6 with hold:10 (transition to hold)**
        cm1.machine_state = MACHINE_PROGRAM_STOP;  // Ensure machine state is correct
        cm1.hold_state = FEEDHOLD_HOLD;             // Set hold state to HOLD
