...
void sr_init_status_report()
{
    ...
    sr.runtime_command_report_pending = false;
    sr.nested_hold_report_count = 0;
...
stat_t sr_status_report_callback()
{
    ...
    // Check for pending runtime command report
    if (sr.runtime_command_report_pending) {
        sr.runtime_command_report_pending = false;
        sr_request_status_report(SR_REQUEST_IMMEDIATE_FULL);
    }
#endif

#if G2_DIAG_ENABLE_NESTED_HOLD_SR_SEQUENCE
    // Just request reports, don't generate them here
    if (cm1.send_nested_hold_report) {
        cm1.send_nested_hold_report = false;

        // Request two immediate reports to create the state transition
        sr.nested_hold_report_count = 2;  // **NEW: Track how many reports to send**
        sr_request_status_report(SR_REQUEST_IMMEDIATE_FULL);
    }
#endif
...
#if G2_DIAG_ENABLE_NESTED_HOLD_SR_SEQUENCE
    // Handle nested-hold report sequence
    if (sr.nested_hold_report_count > 0) {
        // Temporarily switch cm pointer to cm1
        cmMachine_t *saved_cm = cm;
        cm = &cm1;

        if (sr.nested_hold_report_count == 2) {
            // First report: STAT 5/6 with hold:0 (transition)
            cmMachineState saved_machine_state = cm1.machine_state;
            cmFeedholdState saved_hold_state = cm1.hold_state;
            ...
            cm1.machine_state = saved_machine_state;
            cm1.hold_state = saved_hold_state;

            sr.nested_hold_report_count = 1;  // Next report
            sr_request_status_report(SR_REQUEST_IMMEDIATE_FULL);  // Request second report

        } else {  // sr.nested_hold_report_count == 1
            // Second report: STAT 6 with hold:10
            cmMachineState saved_machine_state = cm1.machine_state;
            cmFeedholdState saved_hold_state = cm1.hold_state;
            ...
            sr.nested_hold_report_count = 0;  // Done
        }
    ...
    // Normal report generation continues below
    if ((sr.status_report_request == SR_VERBOSE) ||
        (sr.status_report_verbosity == SR_VERBOSE)) {
        _populate_unfiltered_status_report();
    } else {
        if (_populate_filtered_status_report() == false) {
            return (STAT_OK);
        }
    }
    nv_print_list(STAT_OK, TEXT_MULTILINE_FORMATTED, JSON_OBJECT_FORMAT);
...
stat_t sr_run_unfiltered_status_report(void) {
    nv_reset_nv_list();
    _populate_unfiltered_status_report();
    nv_print_list(STAT_OK, TEXT_MULTILINE_FORMATTED, JSON_OBJECT_FORMAT);
    return (STAT_OK);
